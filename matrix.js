...
const showResult2D = (title, containerId, dataArray) => {
    ...
    // Iterate through each row and column of the dataArray
    for (let i = 0; i < dataArray.length; i++) {
        let tr = document.createElement('tr');
        for (let j = 0; j < dataArray[i].length; j++) {
            let td = document.createElement('td');
            let span = document.createElement('span');
            // Set the content of the span to the value at dataArray[i][j]
    ...
    // Create a caption for the table with the specified title
    let caption = table.createCaption();
    caption.textContent = title;
...
function addMatrices(matrix1, matrix2){
    ...
    // Iterate through each row and column to add corresponding elements
...
const subtractMatrices = function (matrix1, matrix2) { 
    ...
    // Iterate through each row and column to subtract corresponding elements
    for (let i = 0; i < matrix1.length; i++) {
        let row = [];
        for (let j = 0; j < matrix1[0].length; j++) {
            row.push(matrix1[i][j] - matrix2[i][j]);
...
const multiplyMatrices = (matrix1, matrix2) => { 
    ...
    // Check if the matrices can be multiplied (number of columns in matrix1 equals number of rows in matrix2)
